...
export namespace Flamework {
	...
	/**
	 * Registers all of the bundled services, controllers, etc included in @rbxts/flamework.
	 */
	export function registerFlameworkClasses() {
		if (RunService.IsServer()) {
			registerExternalClass(_Bundled.ComponentService);
		} else {
		}
	}
...
/**
 * These are the default services, controllers, etc bundled with @rbxts/flamework.
 *
 * Ideally, in the future, this will be a separate package
 * under a github org (@flamework/component-service, for example)
 */
namespace _Bundled {
	@External()
	@Service({
	...
	export class ComponentService implements OnInit {
		onInit() {
			print("This is from a bundled service [HEYYYYYYYYYYYYYYYYYYYYYYYY]");
		}
	}
}

(Flamework as { Bundled: unknown }).Bundled = _Bundled;
